...
########################################################################################################################
  it "Create Network Zones limit with unexisted network zones id" do
    data = {:resource_class => "Billing::Resource::NetworkGroup",
            :target_id => 0,
            :target_type => "Pack"
    }
...
########################################################################################################################
  ...
  it "Create 'IP Address' limit with pozitive 'Price On' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['prices']['price_ip_on']).to eq(data[:prices][:price_ip_on].to_f)
  end
...
########################################################################################################################
  ...
  it "Create 'IP Address' limit with pozitive 'Price Off' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['prices']['price_ip_off']).to eq(data[:prices][:price_ip_off].to_f)
  end
...
########################################################################################################################
  ...
  it "Create 'Port Speed' limit with pozitive 'Free' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['limits']['limit_rate_free']).to eq(data[:limits][:limit_rate_free].to_f)
  end
  ...
  it "Edit 'Port Speed' limit 'Free' value, set 0" do
    ...
    @br.edit_base_resource(@bp_id, @br.br_id, data)
    response = @br.get_base_resource(@bp_id, @br.br_id)
    expect(response['limits']['limit_rate_free']).to eq(data[:limits][:limit_rate_free])
  end
...
########################################################################################################################
  ...
  it "Create 'Port Speed' limit with pozitive 'Max' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['limits']['limit_rate']).to eq(data[:limits][:limit_rate].to_f)
  end
  ...
  it "Edit 'Port Speed' limit 'Max' value, set as empty (Unlimited)" do
    ...
    @br.edit_base_resource(@bp_id, @br.br_id, data)
    response = @br.get_base_resource(@bp_id, @br.br_id)
    expect(response['limits']['limit_rate']).to eq(data[:limits][:limit_rate])
  end
...
########################################################################################################################
  ...
  it "Create 'Port Speed' limit with pozitive 'Price On' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['prices']['price_rate_on']).to eq(data[:prices][:price_rate_on].to_f)
  end
...
########################################################################################################################
  ...
  it "Create 'Port Speed' limit with pozitive 'Price Off' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['prices']['price_rate_off']).to eq(data[:prices][:price_rate_off].to_f)
  end
...
########################################################################################################################
  ...
  it "Create 'Data Received' limit with pozitive 'Free' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['limits']['limit_data_received_free']).to eq(data[:limits][:limit_data_received_free].to_f)
  end
  ...
  it "Edit 'Data Received' limit 'Free' value, set 0" do
    ...
    @br.edit_base_resource(@bp_id, @br.br_id, data)
    response = @br.get_base_resource(@bp_id, @br.br_id)
    expect(response['limits']['limit_data_received_free']).to eq(data[:limits][:limit_data_received_free])
  end
...
########################################################################################################################
  ...
  it "Create 'Data Received' limit with pozitive 'Price' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['prices']['price_data_received']).to eq(data[:prices][:price_data_received].to_f)
  end
...
########################################################################################################################
  ...
  it "Create 'Data Sent' limit with pozitive 'Free' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['limits']['limit_data_sent_free']).to eq(data[:limits][:limit_data_sent_free].to_f)
  end
  ...
  it "Edit 'Data Sent' limit 'Free' value, set 0" do
    ...
    @br.edit_base_resource(@bp_id, @br.br_id, data)
    response = @br.get_base_resource(@bp_id, @br.br_id)
    expect(response['limits']['limit_data_sent_free']).to eq(data[:limits][:limit_data_sent_free])
  end
...
########################################################################################################################
  ...
  it "Create 'Data Sent' limit with pozitive 'Price' value > 0" do 
    ...
    response = @br.create_base_resource(@bp_id, data)
    expect(response['prices']['price_data_sent']).to eq(data[:prices][:price_data_sent].to_f)
  end
