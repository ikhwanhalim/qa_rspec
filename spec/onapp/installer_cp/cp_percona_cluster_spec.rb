...
describe 'INSTALLER TESTS(CP + PerconaCluster) ->' do
  before :all do
    ...
    @options = InstallerCp::OPTIONS || '--percona-cluster'
  end
  ...
  context 'db ->' do
    ...
    it 'check_db_is_percona_cluster_c7' do
      skip("CP is not based on CentOS 7") unless @version == 7
      expect(vm.check_db_is_percona_cluster_c7).to be true
    end
    ...
    it 'check rpm percona-cluster' do
      expect(vm.check_rpm_percona_cluster).to be true
    end
  end
