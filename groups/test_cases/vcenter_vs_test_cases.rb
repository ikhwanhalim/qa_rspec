...
class VCenterVsTestCases
  ...
  def executable?
    ...
    (env.virtual_machine.unsuspend; env.virtual_machine.info_update) if env.virtual_machine.suspended
    (env.virtual_machine.unlock; env.virtual_machine.info_update) if env.virtual_machine.locked
    (env.virtual_machine.info_update; env.virtual_machine.start_up if !env.virtual_machine.booted) if !env.virtual_machine.up?
...
class VCenterVsPowerOptions < VCenterVsTestCases
  def execute
    ...
    Log.error('Virtual serve not stopped after power off') if !env.virtual_machine.down?

    env.virtual_machine.start_up
    Log.error('Virtual serve not running after startup') if !env.virtual_machine.up?

    env.virtual_machine.shut_down
    Log.error('Virtual serve not stopped after shut down') if !env.virtual_machine.down?

    env.virtual_machine.start_up
    Log.error('Virtual serve not running after startup') if !env.virtual_machine.up?

    env.virtual_machine.reboot
    Log.error('Virtual serve not running after reboot') if !env.virtual_machine.up?
  end
end
class VCenterVsSuspendOptions < VCenterVsTestCases
  def execute
    ...
    Log.error('Virtual serve not stopped after suspend') if !env.virtual_machine.down?
...
class VCenterVsRebuildOperation < VCenterVsTestCases
  ...
  def execute
    ...
    Log.error('Virtual serve not running after rebuild') if !env.virtual_machine.up?
  end
